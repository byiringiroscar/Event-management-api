...
class PhotoChallengeList(ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if PhotoChallenge.objects.filter(user=self.request.user, event=serializer.validated_data['event']).exists():
            return Response({"message": "You have already participated in this photo challenge"}, status=status.HTTP_400_BAD_REQUEST)
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
        return Response({"message": "You have successfully participated in this photo challenge"}, status=status.HTTP_201_CREATED, headers=headers)
